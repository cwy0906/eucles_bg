...
export default class extends Controller {
  ...
  connect() {
    this.chartOptionsTemplateValue = {id:"usd_period_line_chart",library: {plugins: {annotation: {annotations:[{type: 'line',yMin: 31.0 ,yMax: 31.0 ,borderColor:'red',borderDash: [6, 6],borderDashOffset: 0,borderWidth:1,label:{backgroundColor: 'rgba(244, 152, 152, 0.5)',borderWidth: 0,content: "mean(31.0)",position: '0%',yAdjust: -15,display: true,font: {size: 10},},}]}}},zeros:true,precision:5,xtitle:'Date',ytitle:'NTD/USD', min:null, max: null};
    this.load6mChart();
  }

  load6mChart() {
    fetch(this.urlValue+"?period=6m", {headers:{"Accept":"application/json"}}).then(response => response.json()).then(data => {
      this.periodInfoTarget.textContent = data["usd_query_date"]["start"]+" ~ "+data["usd_query_date"]["end"]

      let mean_value = data["recent_period_usd_rate_result"]["mean_rate"]
      let options    = this.getChartOptionsTemplate(mean_value);

      new Chartkick.LineChart(this.chartContainerTarget, [
        { name: "USD - spot buying", data: data["recent_period_usd_rate_result"]["historical_rate"] }
      ], options);
